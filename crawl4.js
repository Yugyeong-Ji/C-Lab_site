...
const getHtml = async () => {
  try {
    return await axios.get("https://news.naver.com/main/list.naver?mode=LS2D&mid=shm&sid1=105&sid2=731", { responseType: "arraybuffer" });
  } catch (error) {
    console.error(error);
  }
};
...
getHtml()
  .then(html => {
    ...
    const fs = require("fs");
    const dataArr = [];
    const dataPath = "./MobileNews.json";
